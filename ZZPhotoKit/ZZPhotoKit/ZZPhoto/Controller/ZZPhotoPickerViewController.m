...
- (UIButton *)doneBtn{
    if (!_doneBtn) {
        _doneBtn = [[UIButton alloc]initWithFrame:CGRectMake(ZZ_VW - 50, 0, 50, 44)];
        [_doneBtn addTarget:self action:@selector(done) forControlEvents:UIControlEventTouchUpInside];
        _doneBtn.titleLabel.font = ButtonFont;
        [_doneBtn setTitleColor:ButtonColor forState:UIControlStateNormal];
        [_doneBtn setTitleColor:[UIColor lightGrayColor] forState:UIControlStateDisabled];
        _doneBtn.enabled=NO;
...
- (UIButton *)previewBtn{
    if (!_previewBtn) {
        _previewBtn = [[UIButton alloc]initWithFrame:CGRectMake(5, 0, 50, 44)];
        [_previewBtn addTarget:self action:@selector(preview) forControlEvents:UIControlEventTouchUpInside];
        _previewBtn.titleLabel.font = ButtonFont;
        [_previewBtn setTitleColor:ButtonColor forState:UIControlStateNormal];
...
- (UILabel *)totalRound{
    if (!_totalRound) {
        _totalRound = [[UILabel alloc]initWithFrame:CGRectMake(ZZ_VW - 70, 10, 22, 22)];
        if (self.roundColor == nil) {
            _totalRound.backgroundColor = ButtonColor;
        }else{
...
- (void)selectPhotoAtIndex:(NSInteger)index{
        ...
        if (photo.isSelect == NO) {
            if (self.selectArray.count + 1 > _selectNum) {
                ...
                if ([self.datas CheckIsiCloudAsset:photo.asset] == YES) {
                    ...
                    [self.selectArray insertObject:photo atIndex:self.selectArray.count];
    ...
    _previewBtn.enabled=self.selectArray.count>0;
    _doneBtn.enabled=self.selectArray.count>0;
    _totalRound.hidden=self.selectArray.count==0;
}
